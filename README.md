# Trades-tracker
Simple discord bot that can be used to track options, stock and futures trades, using Tastytrade API to fetch live data and sqlite3 to store all trades and its trims in a local database.
In a .env file incluse your DISCORD_BOT_TOKEN, TASTYTRADE_USERNAME, TASTYTRADE_PASSWORD, DISCORD_CHANNEL_ID, TASTYTRADE_CLIENT_SECRET, TASTYTRADE_REFRESH_TOKEN
```
pip install -r requirements.txt

python trade_tracker.py
```
BTO /ES @ m
STC /ES @ 6000
BTO NDX 2/12/26 25000C @ 15.0
BTO SPX 2/12/26 6900C @ M

stats myuserid weekly all
